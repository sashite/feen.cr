module FEEN
  module Dumper
    ...
    class Board
      ...
      def initialize(@indexes, board : Hash(Int32, String))
      ...
      private def length
        @indexes.reduce(1) { |i, index| i * index }
      end

      private def unflatten(squares, remaining_indexes)
        return row(squares) if remaining_indexes.size == 1

        squares
          .each_slice(squares.size // remaining_indexes[0])
          .to_a
          .map { |sub_squares| unflatten(sub_squares, remaining_indexes[1..]) }
          .join("/" * remaining_indexes.size.pred)
      end

      private def row(squares)
        squares
          .map { |square| square.nil? ? 1 : square }
          .join(",")
          .gsub(/1,[1,]*1/) { |str| str.split(",").size }
      end
    end
  end
end
